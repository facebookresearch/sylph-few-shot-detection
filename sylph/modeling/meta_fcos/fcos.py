...
class MetaFCOS(nn.Module):  # TODO: convert it to Base Few Shot Detector Head
    ...
    def forward(
        ...
        if self.yield_proposal:
            results["features"] = {f: b for f, b in zip(
                self.in_features, bbox_towers)}
...
class MetaFCOSHead(BaseFewShotDetectorHead):
    ...
    def _evaluate_with_base_class_codes(self, eval_dataset_name: str):
        if "all" not in eval_dataset_name:
            print(
                f"tried to eval with base class codes, but the dataset split is not 'all'")
            return None, None
        if self.base_weight is not None and self.base_bias is not None:
            ...
            base_dataset_id_to_continous_id = MetadataCatalog.get(
                base_dataset_name).thing_dataset_id_to_contiguous_id
            # training data classes
            novel_dataset_id_to_continous_id = MetadataCatalog.get(
                novel_dataset_name).thing_dataset_id_to_contiguous_id
            # combine base class codes with current class_codes

            current_dataset_id_to_continous_id = MetadataCatalog.get(
                eval_dataset_name).thing_dataset_id_to_contiguous_id
            ...
            new_weights = torch.zeros(N, self.base_weight.size(
                1), self.base_weight.size(2), self.base_weight.size(3))
    ...
    def _preload_cls_logits_weights(self):
        ...
        if self.base_weight is not None and self.base_bias is not None:
            ...
            new_bias = self.cls_logits.bias.data
            current_dataset_id_to_continous_id = MetadataCatalog.get(
                self.cfg.DATASETS.TRAIN[0]).thing_dataset_id_to_contiguous_id
            ...
            with torch.no_grad():
                ...
                print(
                    f"done Loading pretrained weights on cls_logits, number of classes: {self.cls_logits.weight.data.size(0)}")
        return
    ...
    def _init_base_detector_config(self, name: Optional[str] = None):
        ...
        print(
            f"eval with clss codes path: {self.evaluate_with_base_class_codes}")
    ...
    def forward_base_train(self, x, top_module=None, yield_bbox_towers=False):
        ...
        for level, feature in enumerate(x):
            ...
            if not self.training and self.evaluate_with_base_class_codes:
                new_weight, new_bias = self._evaluate_with_base_class_codes(
                    self.cfg.DATASETS.TEST[0])
                if new_weight is not None:
                    logger.info(
                        f"evaluating using new weight and new bias, in total {new_weight.size(0)}")
                    logits.append(self.eval_conv(
                        feature=cls_tower, weight=new_weight, bias=new_bias))
                else:
    ...
    def forward(
        ...
        if (not self.training) and self.evaluate_with_base_class_codes:
            ...
            new_weights, new_bias = self._evaluate_with_base_class_codes(
                self.cfg.DATASETS.TEST[0])
            if new_weights is not None:
                print(
                    f"evaluating using new weight and new bias, in total {new_weights.size(0)}")

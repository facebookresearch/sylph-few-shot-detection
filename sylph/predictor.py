...
class SylphPredictor:
    ...
    def __init__(self, config_file: str,  weight_path: str, class_code_path: str, runner_name: str = "sylph.runner.MetaFCOSRunner", test_dataset_names: Dict = None):
        ...
        # if there are multiple novel datasets, only perserve the last one
        for split, dataset_name in test_dataset_names.items():
            if split == "all":
                self.dicts = DatasetCatalog.get(dataset_name)
            self.metadatas[split] = MetadataCatalog.get(dataset_name)
            if split == "all":
                self.class_codes[split] = self._get_datasets_class_codes(
                    self.metadatas[split], dataset_name)
    ...
    def _generate_class_code_from_dataset(self, dataset_name: str, shot: int = 10):
        ...
        data_loader = MetaFCOSRunner.build_episodic_learning_detection_test_support_set_loader(
        ...
        output_dir = _get_inference_dir_name(
            self.cfg.OUTPUT_DIR, "inference", dataset_name)
        logger.info(
            f"Save class codes using {self.cfg.MODEL.META_LEARN.EVAL_SHOT} shots examples in {output_dir}")
        class_codes = inference_on_support_set_dataset(
            self.model, data_loader=data_loader, output_dir=output_dir)
        class_codes = format_class_codes_shared(
            class_codes, device=self.model.device)
    ...
    def _generate_class_codes_from_a_support_set(self, support_set: List[Dict]):
        raise NotImplementedError(
            "_generate_class_codes_from_a_support_set is not implemented")
    ...
    def _get_datasets_class_codes(self, metadata: Dict, dataset_name: str):
        ...
        for class_name in classes:
            ...
            class_codes.append(
                torch.load(PathManager.open(
                    file, mode="rb"), map_location="cpu")
            )
    ...
    def _few_shot_inference(self, inputs: List[Dict], class_codes: Dict[str, torch.Tensor], visualize=False, output_dir=None):
        ...
        with torch.no_grad():
            for ids, dic in enumerate(inputs):
                logger.info(
                    "prediction progress: {}/{}".format(ids, num_examples))
                img_rgb = PathManagerImgLoader(
                    dic["file_name"], img_color="RGB")
                ...
                if visualize:
                    ...
                    PathManagerImgSave(Image.fromarray(
                        out, "RGB"), output_filename)
                total_examples += 1
